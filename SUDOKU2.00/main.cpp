...
bool resolver_sudoku(ReglasSudoku& sudoku, int fila, int col) {

    bool terminado = false;
    int contador;
    do {
        contador = sudoku.dame_contador();
        sudoku.autocompletar();
    } while (sudoku.dame_contador() < contador);
    
    if (!sudoku.terminado()) {

        int c = -1, f = -1, k = 0;
        ReglasSudoku snaposhot = sudoku;
        int i = 0;
        int j = 0;
        bool encontrado = false;
        while (i < 9 && !terminado) {
            while (j < 9 && !terminado) {
                if (sudoku.dame_celda(i, j).es_vacia()) {
                    f = i;
                    c = j;
                    encontrado = true;
                }
        ...
        while (i <= 9 && encontrado) {
            if (sudoku.es_valor_posible(f, c, i)) {
                sudoku.pon_valor(f, c, i);

                if (resolver_sudoku(sudoku, f, c)) {
                    terminado = true;
                }
                else {
                    sudoku = snaposhot;
    ...
    while (am) {
        ...
        while (!sudoku_play.terminado() && !salir) {
                ...
                case 6: 
                    f = 0;
                    c = 0;
                    resolver_sudoku(sudoku_play, f, c);
                    reset(r);
                    break;
                case 7:
                    char save;
                    ...
                    if (save == 'Y') {
                        ...
                        if (!insert) {
                            cout << "DO YOU WANNA DELETE ALL THE PROGRESS OF THE GAME " << sudoku_play.dame_ID() << "? Y or N: ";
                            cin >> save;
